...
def custom_dependency_parameter(param: Parameter) -> Parameter:
    ...
    """
    dependency = param.custom_dependency.configured_for(param)
    return Parameter(
        name=param.name,
        kind=Parameter.KEYWORD_ONLY,
...
#: One resolved dependency per callable it resolves. fast-depends caches what it has
#: resolved by callable, so two parameters depending on the same function have to end up
#: on the same one, or `use_cache` would call it once per parameter.
_resolved: Dict[Tuple[Callable[..., Any], bool, bool], Dependency] = {}
...
def resolve_dependencies(dependency: Dependency) -> Dependency:
    ...
    key = (dependency.dependency, dependency.use_cache, dependency.cast)
    already_resolved = _resolved.get(key)
