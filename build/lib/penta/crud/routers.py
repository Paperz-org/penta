...
class CRUDRouter(
    Generic[
        ModelType, CreateSchemaType, ReadSchemaType, UpdateSchemaType, FilterSchemaType
    ]
):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._model_pk_python_type = self._get_pk_python_type(model)
        ...
        model_name = cast(
            ...
            (
                ...
                if model._meta.verbose_name_plural is None
    ...
    @staticmethod
    def _get_pk_python_type(model: type[ModelType]) -> type:
        """
        The python type the primary key is exposed as in the path of the endpoints.
        """
        pk_python_type = PENTA_TYPES_MAP.get(model._meta.pk.get_internal_type(), int)
        if not isinstance(pk_python_type, type):
            msg = (
                f"The primary key of {model.__name__} is a"
                f" {model._meta.pk.get_internal_type()}, which is not usable as a"
                " path parameter."
            )
            raise ConfigError(msg)
        return pk_python_type
    ...
    def _generate_create_schema(self) -> type[Schema]:
        ...
        return generate_schema(
            self._model,
            name=f"{self._model.__name__}Create",
    ...
    def _generate_filter_schema(self) -> type[FilterSchemaType]:
        ...
        filter_schema: type[FilterSchemaType] = create_model(
            ...
            **{
                field_name: (field_types[field_name], Field(default=None))
