...
from penta.signature.utils import get_args_names, has_kwargs
...
class DjangoGetter:
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        resolver = self._schema_cls._penta_resolvers.get(key)
        if resolver:
...
class ResolverMetaclass(ModelMetaclass):
    _penta_resolvers: Dict[str, Resolver]
    ...
    def __new__(cls, name, bases, namespace, **kwargs):
        ...
        for base in reversed(bases):
            base_resolvers = getattr(base, "_penta_resolvers", None)
            if base_resolvers:
        ...
        result._penta_resolvers = resolvers
        return result
