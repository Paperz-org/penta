...
class BaseCustom(_Depends[..., Optional[T]], Generic[T]):
    ...
    def configure(self, param: "Parameter") -> None:
        """
        Bind the dependency to the view parameter it is annotating.
        """
        if self.param_name is None:
            self.param_name = param.name
        self.annotation_type = _concrete_type(param.annotation_type)
        self._adapter = TypeAdapter(self.annotation_type)
